...
$result = $conn->query($sql);  //query用来运行sql指令
...
if ($row['sex']==1) {
  ...
  $sexx = '女';
    ...
    echo"<tr>";
    //头像是默认头像 不可编辑
    echo"<tr ><th>用户名</th><td>{$row['uname']}</td></tr> <tr><th >性别</th><td>{$sexx}</td></tr> <tr><th >头像</th><td><img src=' touxiang/Dingyang.jpg ' width='80px'></td></tr> <tr><th >密码</th><td>{$row['pwd']}</td></tr> <tr><th >手机号</th> <td>{$row['tel']}</td></tr> <tr><th >邮箱</th> <td>{$row['email']}</td></tr>";
    echo"</table>";
}
